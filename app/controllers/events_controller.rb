class EventsController < ApplicationController
  ...
  def refreshlist
    ...
    if params[:search] != (0).to_s
      @events = Event.near(request.location.city + ", " + request.location.country  , 100).where("name LIKE ? AND start_time < ? AND start_time > ?", '%'+ q + '%', Time.now + 10.days, Time.now - 1.days).limit(10).offset(@offset).order('start_time ASC, atenders DESC')
